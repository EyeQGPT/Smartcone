...
/**
 * Streams the chat completion by fetching a model name from Firestore
 * and opening a stream connection to the Next.js API route.
 * @param input The user's prompt string.
 * @param onChunkReceived Callback function to handle each received chunk of the message.
 ...
 * and opening a stream connection to the Next.js API route.
 * * FIXES INCLUDED:
 * 1. Uses fetch with a ReadableStream reader for chunks.
 * 2. Implements logic to strip {"message":" at the start and "} at the end 
 * of the stream, which was causing the UI display issue.
 * 3. Accumulates the full response to save to history upon completion.
 * * @param input The user's prompt string.
 * @param onChunkReceived Callback function to handle each received chunk of the message.
 * @returns A promise that resolves to the complete AI response text.
 */
export const streamChatCompletion = async (
    ...
    try {
        const snapshot = await getDocs(collection(db, "Model"));
        if (snapshot.docs.length > 0) {
            modelName = snapshot.docs[0].get('Name');
        } else {
            console.error("No model configured in the 'Model' collection.");
            throw new Error(`DB ERROR! Status: No model found in Firestore`);
        }
    } catch (e) {
        console.error('Error fetching model name: ', e);
        throw e;
    }
    ...
    try {
        const response = await fetch('/api/hello', {
            ...
            body: JSON.stringify({
                input: input,
                model: modelName,
                // Indicate to the backend that we expect a stream
                stream: true, 
            }),
            signal: AbortSignal.timeout(10 * 60000), // 10-minute timeout for the full stream
        });

        if (!response.ok) {
            const errorText = await response.text();
            console.error('HTTP Error in streamChatCompletion:', errorText);
        ...
        const startingWrapperRegex = /^(\s*\{?"?message"?\s*:\s*"?)/i;
        const endingWrapperRegex = /("|"?\s*\}?)$/i;
        ...
        while (!done) {
            ...
            if (value) {
                ...
                // 3. FIX: Strip JSON Wrapper
                if (fullResponse.length === 0) {
                    // Strip {"message": " at the very beginning of the stream
                    chunk = chunk.replace(startingWrapperRegex, '');
                }
                
                if (done) {
                    // Strip "} at the very end of the stream
                    chunk = chunk.replace(endingWrapperRegex, '');
                }
                
                // Update buffers
                fullResponse += chunk; 
                
                // Send clean chunk to the UI
                onChunkReceived(chunk);
            }
        }
        
        // 4. Save conversation history after the stream is complete
        await saveQuestionResponse(input, fullResponse); 
